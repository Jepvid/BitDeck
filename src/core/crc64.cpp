// CRC64 table and algorithm ported verbatim from Shipwright/libultraship's
// StrHash64.cpp (https://github.com/HarbourMasters/Shipwright), itself
// adapted from libcore's Crc.cpp (Copyright (c) 2006 Anton Samokhvalov,
// MIT-style license) with contributions tracing back to zlib (Copyright (C)
// 1995-2004 Jean-loup Gailly and Mark Adler) and KeePass (Copyright (c) 2003
// Dominik Reichl). Redistributed here under the same permissive terms.
...
constexpr uint64_t kInitialCrc64 = 0xffffffffffffffffULL;
...
uint64_t crc64(const std::string& text) {
    // No final complement here, unlike Shipwright's buffer-hashing crc64()/update_crc64().
    uint64_t crc = kInitialCrc64;
    for (unsigned char c : text) {
        crc = kCrc64Table[static_cast<uint8_t>(crc >> 56) ^ c] ^ (crc << 8);
    }
    return crc;
}
