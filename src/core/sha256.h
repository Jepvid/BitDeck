...
inline std::array<uint8_t, 32> sha256(const std::vector<uint8_t>& input) {
    ...
    for (size_t chunkStart = 0; chunkStart < data.size(); chunkStart += 64) {
        uint32_t w[64];
        for (int i = 0; i < 16; ++i) {
            w[i] = (static_cast<uint32_t>(data[chunkStart + i * 4 + 0]) << 24) |
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            uint32_t ch = (e & f) ^ (~e & g);
            uint32_t temp1 = hh + s1 + ch + detail::kSha256RoundConstants[static_cast<size_t>(i)] + w[i];
    ...
    for (int i = 0; i < 8; ++i) {
        digest[static_cast<size_t>(i) * 4 + 0] = static_cast<uint8_t>((h[i] >> 24) & 0xFF);
